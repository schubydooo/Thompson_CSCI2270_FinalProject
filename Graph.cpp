...
void Graph::shortestCompleteTraversal(string startCity)
{
    ...
    if(sourceIndex == -1)
    {
        cout << "That city does not appear to be an option" << endl;
        return;
    }
    ...
    vertices[sourceIndex].visited = true;
    vertices[sourceIndex].distance = 0;            //Find starting city, push it into vector containing "solved" nodes
    solved.push_back(&vertices[sourceIndex]);     //where solved means distance to it has been found
    ...
    " The order of cities you will visit is: ";
    for(int i = finalPath.size()-1; i > 0; i--){
        cout<<finalPath.at(i)->name << ", ";
    }
    cout<<finalPath.at(0)->name;
